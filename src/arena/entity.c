        ...
        const struct sage_entity_vtable *vt)
{
    ...
    if (sage_likely (vt)) {
        ctx->vt.update = sage_likely (vt->update) ? vt->update 
                : &update_default2;
...
extern inline sage_entity *_sage_entity_copy(const sage_entity *ctx);


extern inline void _sage_entity_free(sage_entity **ctx);
...
extern inline enum sage_object_id _sage_entity_id_object_(
